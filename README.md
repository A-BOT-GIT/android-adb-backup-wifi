...
3. 在“连接地址”中直接粘贴 `ip:port`，或粘贴系统界面的整段地址文本后点击“智能连接”。
4. 设备出现在列表后即可像 USB 设备一样备份或恢复。

如果当前已经通过 USB 连上设备，也可以不填连接地址，直接点击“智能连接”。程序会尝试自动切换到 `tcpip 5555`、探测设备局域网 IP，并完成 Wi‑Fi 连接。

Android 11+ 还可以直接使用“二维码配对”：

1. 在程序中点击“二维码配对”，弹出二维码窗口。
2. 在手机的“开发者选项 -> 无线调试 -> 使用二维码配对设备”中扫码。
3. 程序会监听配对服务，自动执行 `adb pair`，并在可能时自动完成 `adb connect`。
